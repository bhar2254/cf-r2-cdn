export default {
	async fetch(request, env) {
		const url = new URL(request.url);
		const path = url.pathname;

		if (path === "/" || path === "/index.html") {
			return serveLandingPage();
		}

		// Route: /images/<path-to-image>
		if (path.startsWith("/images/")) {
			const imagePath = path.slice("/images/".length);
			return await fetchImage(env, imagePath);
		}

		// Route: /def/<default-image>/<path-to-image>
		if (path.startsWith("/def/")) {
			const [, , defaultImage, ...imagePathParts] = path.split("/");
			const imagePath = imagePathParts.join("/");
			return await fetchWithDefault(env, imagePath, defaultImage);
		}

		return new Response("Invalid request", { status: 400 });
	},
};

// Serves a basic landing page explaining the CDN
function serveLandingPage() {
	const html = `
	<!DOCTYPE html>
	<html>
	<head>
		<title>R2 CDN Gateway</title>
		<style>
			body { font-family: sans-serif; padding: 2rem; background: #f9f9f9; }
			h1 { color: #2c3e50; }
			code { background: #eef; padding: 0.2rem 0.4rem; border-radius: 4px; }
		</style>
	</head>
	<body>
		<h1>Welcome to the R2 CDN Gateway</h1>
		<p>This Worker serves images directly from a Cloudflare R2 bucket via the following endpoints:</p>
		<ul>
			<li><code>/images/&lt;path-to-image&gt;</code> – Fetch a direct image from the bucket.</li>
			<li><code>/def/&lt;default-name&gt;/&lt;path-to-image&gt;</code> – Fetch an image with fallback logic to a default.</li>
		</ul>
		<p>All images are served with appropriate cache headers. This setup is ideal for websites and applications that need fast, resilient image delivery.</p>
	</body>
	</html>
	`;
	return new Response(html, {
		headers: { "Content-Type": "text/html; charset=UTF-8" },
	});
}

// Fetches an image from the R2 bucket
async function fetchImage(env, imagePath) {
	try {
		const object = await env.R2_BUCKET.get(imagePath);
		if (!object) return null;

		const contentType = getContentType(imagePath) || "application/octet-stream";
		return new Response(object.body, {
			headers: {
				"Content-Type": contentType,
				"Cache-Control": "public, max-age=86400", // 1 day
			},
		});
	} catch (error) {
		return new Response("Error fetching image", { status: 500 });
	}
}

// Fetches an image with fallback logic
async function fetchWithDefault(env, imagePath, defaultImage) {
	let response = await fetchImage(env, imagePath);

	if (!response) {
		const yearRegex = /(\/)\d{4}(\/|$)/;
		if (yearRegex.test(imagePath)) {
			const newPath = imagePath.replace(yearRegex, "$1");
			response = await fetchImage(env, newPath);
		}
	}

	if (!response) {
		response = await fetchImage(env, `def/${defaultImage}.webp`);
	}

	if (!response) {
		response = await fetchImage(env, "def/default.webp");
	}

	return response || new Response("Image not found", { status: 404 });
}

// Detects content type based on file extension
function getContentType(path) {
	const ext = path.split(".").pop().toLowerCase();
	const types = {
		webp: "image/webp",
		jpg: "image/jpeg",
		jpeg: "image/jpeg",
		png: "image/png",
		gif: "image/gif",
		svg: "image/svg+xml",
		ico: "image/x-icon",
	};

	return types[ext] || null;
}
